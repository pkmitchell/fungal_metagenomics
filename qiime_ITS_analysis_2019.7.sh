...
echo "Running ITSxpress Trimming"
qiime itsxpress trim-pair-output-unmerged\
  --i-per-sample-sequences $output_dirvar/demux-paired-end.qza \
  --p-region ITS1 \
  --p-taxa F \
  --p-cluster-id 1 \
  --p-threads $nthreads \
  --o-trimmed $output_dirvar/itsxpress-trimmed.qza

## Dada2 denoise (error correction, chimera check, trimming)
echo "Step 4: Dada2 denoise and representative sequence picking"
qiime dada2 denoise-paired \
  --i-demultiplexed-seqs $output_dirvar/itsxpress-trimmed.qza \
  --p-trim-left-f 0 \
  --p-trim-left-r 0 \
  --p-trunc-len-f 0 \
  --p-trunc-len-r 0 \
